...
export function commandsView(commands: ProcessedCommandModel[], timeZone?: string): string {
  ...
  return `<header class="page-head presentation-peak" data-presentation="peak"><h1 data-focus-fallback tabindex="-1">Command ingestion</h1><p>Observed commands, including requests that were refused or produced no job.</p></header><section class="panel presentation-inset" data-presentation="inset">${table}</section>`;
}
...
export function auditView(actions: OperatorActionModel[], timeZone?: string): string {
  ...
  return `<header class="page-head presentation-peak" data-presentation="peak"><h1 data-focus-fallback tabindex="-1">Operator audit</h1><p>Every manual action is recorded with its target and effect.</p></header><section class="panel presentation-inset" data-presentation="inset">${table}</section>`;
}
