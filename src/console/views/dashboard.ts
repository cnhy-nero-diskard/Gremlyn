...
export function dashboardRegions(model: DashboardModel): {
  health: string;
  repositories: string;
  jobs: string;
} {
  ...
  return {
    ...
    repositories: `<h2>Repositories</h2>${repositoryCards(model.repositories)}`,
...
export function dashboardView(model: DashboardModel): string {
  const regions = dashboardRegions(model);
  return `<h1>Dashboard</h1><p class="sr-status" data-live-status role="status">Live updates are connected when supported.</p><div id="health-region">${regions.health}</div><section class="panel" id="repositories">${regions.repositories}</section><div id="job-lanes">${regions.jobs}</div>`;
}
