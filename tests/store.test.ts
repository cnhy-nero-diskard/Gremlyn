...
test("migrations apply cleanly against a fresh database", () => {
  ...
  const applied = store.db.prepare("SELECT id FROM schema_migrations ORDER BY id").all() as {
    id: string;
  }[];
  assert.deepEqual(
    applied.map((r) => r.id),
...
test("migrations are idempotent against an already-migrated database", () => {
  ...
  first.db
    .prepare("INSERT INTO log_entries (at, level, event) VALUES (?, ?, ?)")
    .run(new Date().toISOString(), "info", "marker");
  first.close();
  ...
  const marker = second.db.prepare("SELECT event FROM log_entries").get() as { event: string };
