...
- [x] 6.4 Run `node --import tsx --test tests/console.test.ts tests/provider-catalog.test.ts`, `npm run build`, `npm run lint`, `npm run format:check`, and `npm test`; verify all commands pass without adding a client framework, bundler, or runtime dependency.
