...
- [x] 1.1 Extend `npm run probe:agent` with a seeded-run mode that copies a declared
      file list into the fresh `--data-dir` before invoking, and verify the probe
      reports the copied set alongside the run outcome
- [x] 1.2 Determine empirically which files an isolated `--data-dir` needs to
...
- [x] 3.2 Extend the startup agent check to verify the credential source exists and
      is readable, and verify a missing or unreadable source refuses startup with a
      configuration error and accepts no jobs
- [x] 3.3 Verify the credential source is never written to — a full job run leaves
      the operator's authenticated agent installation byte-identical

## 4. Seed and scope the credential

- [x] 4.1 Seed the confirmed file set into each attempt's `--data-dir` before
...
- [x] 4.2 Verify no credential value reaches the process argument vector or the
      agent environment, extending the existing secret-isolation assertions
- [x] 4.3 Verify the seeded credential is removed on every terminal path — success,
      failure, timeout, and cancellation — leaving no credential material on disk
- [x] 4.4 Extend the startup interrupted-job sweep to remove stale attempt
...
- [x] 5.1 Verify two concurrent attempts each authenticate successfully and neither
...
- [x] 5.2 Verify a seeded run and an unseeded run against the same source directory
      produce independent session state, so seeding introduces no shared mutable path

## 6. Documentation and acceptance

- [x] 6.1 Add the credential-source field to `config.example.yaml` with the
      conventional default and a note that it is read-only, and verify the loader
      parses the example successfully
- [x] 6.2 Document in the README how to authenticate the agent and where its
...
- [x] 6.3 Run `npm run probe:agent` against the configured provider and model and
      verify both isolated runs complete, closing the finding this change exists to
      fix
- [x] 6.4 Verify task 11.6 of `add-pr-resolution-orchestrator` — the full acceptance
